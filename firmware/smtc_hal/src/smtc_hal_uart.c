...
volatile uint32_t g_uart0_bytes_rx = 0;
volatile uint32_t g_uart0_lines_parsed = 0;
...
static void uart0_handleEvent( app_uart_evt_t *pEvent )
{
    switch( pEvent -> evt_type )
    {
        case APP_UART_DATA_READY:
        {
            ...
            if( g_uart0_bytes_rx < 0xFFFFFFFF ) g_uart0_bytes_rx++;
            ...
            if(( g_rx1_data[0] == '\n' ) && ( g_rx1_len != 0 ))
            {
                ...
                if( g_rx1_line >= 2 )
                {
                    if( g_uart0_lines_parsed < 0xFFFFFFFF ) g_uart0_lines_parsed++;

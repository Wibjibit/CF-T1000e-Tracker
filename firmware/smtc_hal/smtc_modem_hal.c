...
// Updated by the application when it samples the battery. 0xFF means
// "no sample yet" → we return LoRaWAN's "unable to measure" sentinel.
volatile uint8_t g_last_battery_pct = 0xFF;

uint8_t smtc_modem_hal_get_battery_level( void )
{
    uint8_t pct = g_last_battery_pct;
    if( pct == 0xFF )
    {
        return 255; // unable to measure (no sample yet)
    }
    if( pct > 100 )
    {
        pct = 100;
    }
    // Map 0..100 % → LoRaWAN's 1..254 scale. 0 % stays at 1 (lowest, not "external power").
    return ( uint8_t )( 1 + ( ( uint32_t ) pct * 253 ) / 100 );
}

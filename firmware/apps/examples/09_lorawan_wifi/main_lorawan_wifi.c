...
int main( void )
{
    ...
    HAL_DBG_TRACE_MSG( "\n" );
    HAL_DBG_TRACE_INFO( "###### ===== T1000-E WIFI scan example ==== ######\n\n" );
...
static void on_modem_alarm( void )
{
    ...
    {
        ASSERT_SMTC_MODEM_RC( smtc_modem_alarm_start_timer( WIFI_SCAN_TIME ));
    ...
    else if( wifi_scan_status == 1 ) // send data
    {
        ...
        wifi_get_results( modem_radio, app_data_buffer, &app_data_len );
